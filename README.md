...
![Isometric drawing of a freestanding bookshelf, generated from a parametric model](assets/shelf-iso-hero-v2.png)

*Above: generated by `assets/shelf.py` — a true hidden-line isometric from a 3D model. The CNC-ready DXF is extracted from a face of that same model, so the picture and the cut file stay in sync as the design changes.*
